...
function addRating(obj,productid) {
	...
	$('.demo-table #product-'+productid+' li').each(function(index) {
        ...
        $('#product-'+productid+' #rating').val((index+1)); 
		if(index == $('.demo-table #product-'+productid+' li').index(obj)) {  //若迴圈次數=被選中的星星數量則中止
			return false;	
		}
    });
    //使用jquery.ajax()方法
	...
	data:'productid='+productid+'&rating='+$('#product-'+productid+' #rating').val(),  //要送出去的數據，id=被選中的星星id,rating=評分紀錄
	type: "POST",  //發送數據的類型
    success: function(data) {  //請求成功時要運行的函式
        $("#star-rating-count-"+productid).html(data);    //將<div id='star-rating-count-id'>里的所有內容換成 data
        }
	});
}
									...
									<tr valign="top">
										<td width="100%" class="tdrline">
											<div class="category"> 
												<p>預約紀錄</p>
												<ul>
                                                    <li><a href='order_admin.php'>所有預約</a></li>  
													<li><a href="order_admin.php?stat=已付款">已付款</a></li>
													<li><a href="order_admin.php?stat=完成">完成</a></li>
													<li><a href="order_admin.php?stat=取消">取消</a></li>
												</ul>
											</div>
											<hr width="100%" size="1">
